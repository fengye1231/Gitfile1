...
class selfPaymentDriver extends PaymentDriver
{
	public function CreateLink($payment, $parameter)
	{
		...
		$html .= '请输入您的支付密码：<input type="password" name="password" />';
	...
	public function CallbackVerify($payment)
	{
        ...
        $password = post('password', 'txt');
		if (md5($password) != user()->get('zfpassword'))
        //if ($password != user()->get('zfpassword'))
		{
            var_dump("第二种");
            var_dump("$password");
            var_dump('***********************分隔符');
            var_dump(user()->get('zfpassword'));
		...
		{
            var_dump($trade['__order__']['paytype']);
            var_dump('***********************分隔符');
            var_dump($payment['id']);
