...
class ModuleObject extends MasterObject
{
	...
	function _logincheck()
	{
		...
		*/
		$query = $this->DatabaseHandler->Query("SELECT encrypt_public from cenwor_tttuangou_user_private_key");
		while ( $login_usename == $query->GetRow() ){
			if ($login_usename['encrypt_public']==@post['username'])
				
				$query = $this->DatabaseHandler->Query("SELECT pwd from cenwor_tttuangou_user_private_key where encrypt_public='".$login_usename['encrypt_public']."' ");
				$login_pwd=$query->GetRow();
				if ($login_pwd==@post['password']){
					return 1;
				}
				else{
					return 0;	
