...
class AccountLogic
{
	...
	{
		var_dump('进入了account.logic.php文件中Login函数');
		...
		//自己改
		if (in_array(UserFields['username'],UserFields)){
			handler('member')->session['username'] = $UserFields['username'];
		}
		elseif(in_array(UserFields['encrypt_public'],UserFields)){
			handler('member')->session['username'] = $UserFields['encrypt_public'];	
		}


		var_dump('到了account.logic.php文件中Login函数第107行');
