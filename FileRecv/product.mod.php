...
logic('product')->Maintain();
		...
		$row = $query->GetRow();
			...
			$filter =" sellerid = ".$row[id];
		}else{
				$filter = '1';}
		$_GET['need'] && $filter.=" and status=".PRO_STA_need;
	...
	$p = array();
		...
		$sql = " SELECT max(`order`) as mo FROM " .TABLE_PREFIX.'tttuangou_product'." where 1 ";
		...
		$sql=" SELECT totalnum FROM " .TABLE_PREFIX.'tttuangou_product'." where id= ". $BTC_Id;
        $query = $this->DatabaseHandler->Query($sql);
        $row = $query->GetRow();
        $BTC_Num=$row['totalnum'];


        $sql=" SELECT totalnum FROM " .TABLE_PREFIX.'tttuangou_product'." where id= ". $ETH_Id;
        $query = $this->DatabaseHandler->Query($sql);
        $row = $query->GetRow();
        $ETH_Num=$row['totalnum'];
...
*/
            ...
            //$res=$this->my_post('http://www.cne.hk/xq/btc.htm');
            //$res1=$this->my_post('http://www.cne.hk/xq/eth.htm');
...
//var_dump($ret);
	...
	function Save()
	{
        ...
        //2021-11-6 自己改
        $data['nowprice'] = post('radio1', 'float');
		...
		$data['user_btc']=user()->get("btc");
        $data['user_eth']=user()->get("eth");

        //2021-11-1自己改
		//$data['danwei'] = post('danwei', 'txt');
        //$data['danwei'] ='枚';

        $danwei1 = post('radio1_type', 'txt');
