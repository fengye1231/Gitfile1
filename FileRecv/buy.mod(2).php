...
class ModuleObject extends MasterObject
{
	...
	$yanzheng = logic('yanzheng')->GetOne(user()->get('id'));
        ...
        //global $mynowprice;
        $mynowprice = get('nowprice', 'float');
        //$GLOBALS['var1']=$mynowprice;
    ...
    function Checkout_save()
    {
        ...
        $product['nowprice']=post('nowprice', 'float');
        //2021-11-18自己改
        //$order['productprice'] = post('nowprice', 'float');

        //$order['productprice']=(float)$_POST['nowprice'];


        //var_dump('product中nowprice为：');
        //var_dump( "$product['nowprice']");
    ...
    function Order_save()
    {
        ...
        //$me_money = user()->get('btc');
        $me_money = user()->get('hkc');
    ...
    function Pay()
    {
        ...
        if($pid=='47'){
            ...
            //无跳转支付的回调
            $parameter['notify_url']=ini('settings.site_url').'/index.php?mod=me&code=order';
            $parameter['is_buy']=$order['productid'];
        }
