...
class Order_Parser_CallbackLogic extends CallbackLogic
{
	...
	function Parse_TRADE_FINISHED($payment = false)
	{
        ...
        //var_dump("在文件callback.logic.php中的Parse_TRADE_FINISHED函数中");

		$this->master->Messager($error?$error:__('本次交易已经完成！'), '?mod=me&code=order',10);
	}
}
...
class Recharge_Parser_CallbackLogic extends CallbackLogic
{
	...
	function Parse_TRADE_FINISHED($payment = false)
	{
		...
		if($payment['code']=='mobpay')
		{
			...
			{
				logic('pay')->Process($payment, 'TRADE_FINISHED');
			}
		}
		else
		{
