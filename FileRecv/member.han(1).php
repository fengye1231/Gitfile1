...
class MemberHandler 
{
	...
	function FetchMember($id, $pass,$secques='')
	{
		...
		{
			/*
			if ((int) $this->MemberFields['uid']!=-1){
				define("MEMBER_ID",(int) $this->MemberFields['uid']);
			}else{
				define("MEMBER_ID",(int) $this->MemberFields['id']);				
			}
			*/
			define("MEMBER_ID",(int) $this->MemberFields['uid']);
			...
			/*
			if (in_array('username',$this->MemberFields)){
				define("MEMBER_NAME",$this->MemberFields['username']);
			}elseif(in_array('encrypt_public',$this->MemberFields){
				define("MEMBER_NAME",$this->MemberFields['encrypt_public']);
			}
			*/
			define("MEMBER_NAME",$this->MemberFields['username']);
		...
		if($row['statue']==2)
		{
           ...
           {
               define("MEMBER_IDNAME",$row['idname']);

               if( strstr(MEMBER_NAME,"匿名"))
                   {
                       define("MEMBER_IDNAME",MEMBER_NAME);
                   }
	...
	function CheckMember_nm($user,$password,$clsF=true) 
	{
	    //2021/8/24之前
        /*
		$this->SetMember($user);
	    ...
	    if (trim($user)!=''){
				...
				WHERE 
					encrypt_public='{$this->MemberName}'";
				$query = $this->DatabaseHandler->Query($sql);
				$this->MemberFields=$query->GetRow();
				if($this->MemberFields!=false) 
				{
					if($this->MemberFields['pwd']==$password) 
					{				
						define("MEMBER_ID_NM",(int) $this->MemberFields['id']);
						define("MEMBER_NAME_NM",$this->MemberFields['encrypt_public']);	
						
						var_dump("常量MEMBER_ID_NM=");
						var_dump(MEMBER_ID_NM);
