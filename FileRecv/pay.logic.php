...
class PayLogic
{
	...
	public function apiz($code)
	{
		...
		if (!$api)
		{
		    //driver('payment')  返回payment.drv.php文件
	...
	/*
	*[2015-08-18]无跳转支付的几个ajax的后台支持
	*/
	public function querybind($code,$cardno){
		/*
		**查询是否绑定
		*return array
		*/
		return $this->apiz($code)->querybind($cardno);
	}
	public function bindandpay($code,$orderid,$price,$cardno){
		/*
		**绑定并支付
		*return string
		*/
		return $this->apiz($code)->bindandpay($orderid,$price,$cardno);
	}
	public function dosms($code,$orderid,$price,$bindno){
		/*
		**发送短信验证码
		*return array
		*/
		return $this->apiz($code)->dosms($orderid,$price,$bindno);
	}
	public function dopay($code,$orderid,$bindno,$sms,$price){
		/*
		**支付
		*return array
		*/
		return $this->apiz($code)->dopay($orderid,$bindno,$sms,$price);
	}
