...
class ProductLogic
{
	...
	public function display($category = '1')
	{
        ...
        //$product_num=(int)$product['totalnum']-2;
...
function  shenhe($id)
	{
		...
		dbc(DBCMax)->query('UPDATE '.table('product').' SET status='.PRO_STA_Normal.' WHERE  id='.$id.' and   status='.PRO_STA_need)->done();
	    $this->UpdateSTATUS();
	}
	function UpdateSTATUS()
	{
		$now = time();
		$sqls = array(
						'UPDATE '.table('product').' SET status='.PRO_STA_Failed.' WHERE successnum>(virtualnum+totalnum) AND overtime<'.$now.' and  AND status<>'.PRO_STA_need.' and  begintime<'.$now,
						'UPDATE '.table('product').' SET status='.PRO_STA_Finish.' WHERE successnum<=(virtualnum+totalnum) AND overtime<'.$now.' AND status<>'.PRO_STA_need.' and  begintime<'.$now,
						'UPDATE '.table('product').' SET status='.PRO_STA_Normal.' WHERE successnum>(virtualnum+totalnum) AND overtime>'.$now.' AND status<>'.PRO_STA_need.' and  begintime<'.$now,
						'UPDATE '.table('product').' SET status='.PRO_STA_Success.' WHERE successnum<=(virtualnum+totalnum) AND overtime>'.$now.' AND status<>'.PRO_STA_need.' and  begintime<'.$now,
		);
		$r = 0;
		foreach ($sqls as $i => $sql)
		{
	...
	{
		$STA_NAME_MAP = array(
			PRO_STA_Failed => '已结束，失败',
			PRO_STA_Normal => '进行中，',
			PRO_STA_Success => '进行中，',
			PRO_STA_Finish => '已结束，成功',
