...
class AccountLogic
{
	...
	public function Login($username, $password, $keepLogin = true)
	{
		...
		public function Login_nm($username, $password, $keepLogin = true)
	{
	    /*
		$aCheckResult = $this->invaidAccount($username, $password);
		if ($aCheckResult)
		...
		$extend = $exLogin['result'];
	    */
        $check = handler('member')->CheckMember_nm($username, $password);
		if ($check == -1)
		{
			return $this->ErrorInf(__('无法登录，用户密码错误，您可以有至多 5 次尝试。'));
		}
		...
		//2021/8/24之前自己改
        /*
		handler('member')->session['username'] = $UserFields['encrypt_public'];
		$authcode = authcode("{$UserFields['password']}\t{$UserFields['secques']}\t{$UserFields['uid']}", 'ENCODE');
		if ( $keepLogin )
		{
			$expires = (int)ini('settings.cookie_expire') * 86400;
		...
		handler('cookie')->SetVar('cookietime', '2592000', $expires);
		session_start();
		$_SESSION['username_nm'] = $UserFields['encrypt_public'];
		//define("MEMBER_ACCOUNT_NM",(string)handler('member')->session['username']);
		return $this->SuccInf($extend);
        */
        ...
        session_start();
        //$_SESSION['username_nm'] = $UserFields['encrypt_public'];
        $_SESSION['username_nm'] = $UserFields['username'];
        //var_dump("UserFields=");
        //var_dump($UserFields);
        ...
        if (!$id) {
            logic("yanzheng")->Add($uid1, $post);
        } else {
            logic("yanzheng")->Update($id, $post, $uid1);
        }
    ...
    public function Register($username, $password, $mail = '', $phone = '', $qq = '', $noExRegister = false,$zfpassword='')
	{
