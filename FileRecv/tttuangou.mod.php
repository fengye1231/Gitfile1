...
function Execute(){
	...
	function Varshow(){
        ...
        if($result && is_array($result)) {
            $product['default_purchase_cash'] = $result['purchase'];
            $product['default_spot_selling_rate'] = $result['selling'];
        }
		$result1 = getWphj(港币);
        if($result1 && is_array($result1)) {
            $product['default_hkd_purchase_cash'] = $result1['purchase'];
            $product['default_hkd_spot_selling_rate'] = $result1['selling'];
        }
		$result2 = getWphj(欧元);
        if($result2 && is_array($result2)) {
            $product['default_eur_purchase_cash'] = $result2['purchase'];
            $product['default_eur_spot_selling_rate'] = $result2['selling'];
        }
		$result3 = getWphj(日元);
        if($result3 && is_array($result3)) {
            $product['default_jpy_purchase_cash'] = $result3['purchase'];
            $product['default_jpy_spot_selling_rate'] = $result3['selling'];
        }
		$result4 = getWphj(韩国元);
        if($result4 && is_array($result4)) {
            $product['default_krw_purchase_cash'] = $result4['purchase'];
            $product['default_krw_spot_selling_rate'] = $result4['selling'];
        }
		$result5 = getWphj(英镑);
        if($result5 && is_array($result5)) {
            $product['default_gbp_purchase_cash'] = $result5['purchase'];
            $product['default_gbp_spot_selling_rate'] = $result5['selling'];
        }
		$result6 = getWphj(澳大利亚元);
        if($result6 && is_array($result6)) {
            $product['default_aud_purchase_cash'] = $result6['purchase'];
            $product['default_aud_spot_selling_rate'] = $result6['selling'];
        }
		$result7 = getWphj(加拿大元);
        if($result7 && is_array($result7)) {
            $product['default_cad_purchase_cash'] = $result7['purchase'];
            $product['default_cad_spot_selling_rate'] = $result7['selling'];
        }
		$result8 = getWphj(瑞士法郎);
        if($result8 && is_array($result8)) {
            $product['default_chf_purchase_cash'] = $result8['purchase'];
            $product['default_chf_spot_selling_rate'] = $result8['selling'];
        }
		$result9 = getWphj(新西兰元);
        if($result9 && is_array($result9)) {
            $product['default_nzd_purchase_cash'] = $result9['purchase'];
            $product['default_nzd_spot_selling_rate'] = $result9['selling'];
        }
		$result10 = getWphj(新加坡元);
        if($result10 && is_array($result10)) {
            $product['default_sgd_purchase_cash'] = $result10['purchase'];
            $product['default_sgd_spot_selling_rate'] = $result10['selling'];
        }
		$result11 = getWphj(美元);
        if($result11 && is_array($result11)) {
            $product['default_hkc_purchase_cash'] = $result11['purchase'];
            $product['default_hkc_spot_selling_rate'] = $result11['selling'];
        }
		$result12 = getWphj(美元);
        if($result12 && is_array($result11)) {
            $product['default_btc_purchase_cash'] = $result11['purchase'];
            $product['default_btc_spot_selling_rate'] = $result11['selling'];
        }
		$result13 = getWphj(美元);
        if($result13 && is_array($result11)) {
            $product['default_eth_purchase_cash'] = $result11['purchase'];
            $product['default_eth_spot_selling_rate'] = $result11['selling'];
        }
		include(handler('template')->file("@admin/tttuangou_var"));
	}

	function Varshows(){
		$action='?mod=tttuangou&code=varedits';
		$product=ConfigHandler::get('product');
				include (CONFIG_PATH.'settings.php');
		$product['default_imgwidth']=$config['thumbwidth'];
		$product['default_imgheight']=$config['thumbheight'];
		...
		// 
		$product['usa_hkd_purchase_cash'] = "";
		$product['usa_hkd_spot_selling_rate'] = "";
		$product['usa_eur_purchase_cash'] = "";
		$product['usa_eur_spot_selling_rate'] = "";
		$product['usa_jpy_purchase_cash'] = "";
		$product['usa_jpy_spot_selling_rate'] = "";
		$product['usa_gbp_purchase_cash'] = "";
		$product['usa_gbp_spot_selling_rate'] = "";
		$product['usa_aud_purchase_cash'] = "";
		$product['usa_aud_spot_selling_rate'] = "";
		$product['usa_cad_purchase_cash'] = "";
		$product['usa_cad_spot_selling_rate'] = "";
		$product['usa_chf_purchase_cash'] = "";
		$product['usa_chf_spot_selling_rate'] = "";
		$product['usa_nzd_purchase_cash'] = "";
		$product['usa_nzd_spot_selling_rate'] = "";
		$product['usa_sgd_purchase_cash'] = "";
		$product['usa_sgd_spot_selling_rate'] = "";

		// $result = getWphk("人民币");
        // if($result && is_array($result)) {
        //     $product['usa_purchase_cash'] = $result['purchase'];
        //     $product['usa_spot_selling_rate'] = $result['selling'];
        // }else{
        //     $product['usa_purchase_cash'] = "";
        //     $product['usa_spot_selling_rate'] = "";
		// }
		$result_all=getWphk();
		// $result1 = getWphk("港元");
        if($result_all && array_key_exists("港元",$result_all)) {
            $product['usa_hkd_purchase_cash'] = $result_all["港元"]['purchase'];
            $product['usa_hkd_spot_selling_rate'] = $result_all["港元"]['selling'];

        }
		// $result2 = getWphk("欧罗");
        if($result_all && array_key_exists("欧罗",$result_all)) {
            $product['usa_eur_purchase_cash'] = $result_all["欧罗"]['purchase'];
            $product['usa_eur_spot_selling_rate'] = $result_all["欧罗"]['selling'];
		}
		// $result3 = getWphk("日元");
        if($result_all && array_key_exists("日元",$result_all)) {
            $product['usa_jpy_purchase_cash'] = $result_all["日元"]['purchase'];
            $product['usa_jpy_spot_selling_rate'] = $result_all["日元"]['selling'];
		}
		// $result5 = getWphk("英镑");
        if($result_all && array_key_exists("英镑",$result_all)) {
            $product['usa_gbp_purchase_cash'] = $result_all["英镑"]['purchase'];
            $product['usa_gbp_spot_selling_rate'] = $result_all["英镑"]['selling'];
		}
		// $result6 = getWphk("澳元");
        if($result_all && array_key_exists("澳元",$result_all)) {
            $product['usa_aud_purchase_cash'] = $result_all["澳元"]['purchase'];
            $product['usa_aud_spot_selling_rate'] = $result_all["澳元"]['selling'];
		}
		// $result7 = getWphk("加元");
        if($result_all && array_key_exists("加元",$result_all)) {
            $product['usa_cad_purchase_cash'] = $result_all["加元"]['purchase'];
            $product['usa_cad_spot_selling_rate'] = $result_all["加元"]['selling'];
		}
		// $result8 = getWphk("瑞士法郎");
        if($result_all && array_key_exists("瑞士法郎",$result_all)) {
            $product['usa_chf_purchase_cash'] = $result_all["瑞士法郎"]['purchase'];
            $product['usa_chf_spot_selling_rate'] = $result_all["瑞士法郎"]['selling'];
		}
		// $result9 = getWphk("纽元");
        if($result_all && array_key_exists("纽元",$result_all)) {
            $product['usa_nzd_purchase_cash'] = $result_all["纽元"]['purchase'];
            $product['usa_nzd_spot_selling_rate'] = $result_all["纽元"]['selling'];
		}
		// $result10 = getWphk("新加坡元");
        if($result_all && array_key_exists("新加坡元",$result_all)) {
            $product['usa_sgd_purchase_cash'] = $result_all["新加坡元"]['purchase'];
            $product['usa_sgd_spot_selling_rate'] = $result_all["新加坡元"]['selling'];
		}
		// $result11 = getWphk("美元");
        // if($result11 && is_array($result11)) {
        //     $product['usa_hkc_purchase_cash'] = $result11['purchase'];
        //     $product['usa_hkc_spot_selling_rate'] = $result11['selling'];
        // }
		include(handler('template')->file("@admin/tttuangou_var_rmb"));
	}




    function Varshows_nm(){
        $action='?mod=tttuangou&code=varedits';
        $product=ConfigHandler::get('product');
        include (CONFIG_PATH.'settings.php');
        $product['default_imgwidth']=$config['thumbwidth'];
        $product['default_imgheight']=$config['thumbheight'];
        $result = getWphj_nm(美元);
        if($result && is_array($result)) {
            $product['default_purchase_cash'] = $result['purchase'];
            $product['default_spot_selling_rate'] = $result['selling'];
        }
	...
	function Varedits(){
        ...
        $set['default_purchase_cash']=$default_purchase_cash;
        $set['default_spot_selling_rate']=$default_spot_selling_rate;
		$set['default_hkd_purchase_cash']=$default_hkd_purchase_cash;
		$set['default_hkd_spot_selling_rate']=$default_hkd_spot_selling_rate;
		$set['default_eur_purchase_cash']=$default_eur_purchase_cash;
		$set['default_eur_spot_selling_rate']=$default_eur_spot_selling_rate;
		$set['default_jpy_purchase_cash']=$default_jpy_purchase_cash;
		$set['default_jpy_spot_selling_rate']=$default_jpy_spot_selling_rate;
		$set['default_krw_purchase_cash']=$default_krw_purchase_cash;
		$set['default_krw_spot_selling_rate']=$default_krw_spot_selling_rate;
		$set['default_gbp_purchase_cash']=$default_gbp_purchase_cash;
		$set['default_gbp_spot_selling_rate']=$default_gbp_spot_selling_rate;
		$set['default_aud_purchase_cash']=$default_aud_purchase_cash;
		$set['default_aud_spot_selling_rate']=$default_aud_spot_selling_rate;
        $set['default_cad_purchase_cash']=$default_cad_purchase_cash;
		$set['default_cad_spot_selling_rate']=$default_cad_spot_selling_rate;
		$set['default_chf_purchase_cash']=$default_chf_purchase_cash;
		$set['default_chf_spot_selling_rate']=$default_chf_spot_selling_rate;
		$set['default_nzd_purchase_cash']=$default_nzd_purchase_cash;
		$set['default_nzd_spot_selling_rate']=$default_nzd_spot_selling_rate;
		$set['default_sgd_purchase_cash']=$default_sgd_purchase_cash;
		$set['default_sgd_spot_selling_rate']=$default_sgd_spot_selling_rate;
		$set['default_hkc_purchase_cash']=$default_hkc_purchase_cash;
		$set['default_hkc_spot_selling_rate']=$default_hkc_spot_selling_rate;
		$set['default_btc_purchase_cash']=$default_btc_purchase_cash;
		$set['default_btc_spot_selling_rate']=$default_btc_spot_selling_rate; 
		$set['default_eth_purchase_cash']=$default_eth_purchase_cash;
		$set['default_eth_spot_selling_rate']=$default_eth_spot_selling_rate;      
		//
		if(!empty($usa_purchase_cash)){
		$set['usa_purchase_cash']=$usa_purchase_cash;
		}
		if(!empty($usa_purchase_cash)){
        $set['usa_spot_selling_rate']=$usa_spot_selling_rate;
		}
		if(!empty($usa_hkd_purchase_cash)){
		$set['usa_hkd_purchase_cash']=$usa_hkd_purchase_cash;
		}
		if(!empty($usa_hkd_spot_selling_rate)){
		$set['usa_hkd_spot_selling_rate']=$usa_hkd_spot_selling_rate;
		}
		if(!empty($usa_eur_purchase_cash)){
		$set['usa_eur_purchase_cash']=$usa_eur_purchase_cash;
		}
		if(!empty($usa_eur_spot_selling_rate)){
		$set['usa_eur_spot_selling_rate']=$usa_eur_spot_selling_rate;
		}
		if(!empty($usa_jpy_purchase_cash)){
		$set['usa_jpy_purchase_cash']=$usa_jpy_purchase_cash;
		}
		if(!empty($usa_jpy_spot_selling_rate)){
		$set['usa_jpy_spot_selling_rate']=$usa_jpy_spot_selling_rate;
		}
	// 	if(!empty($usa_purchase_cash)){
	// 	$set['usa_krw_purchase_cash']=$usa_krw_purchase_cash;
	// }
	// 	if(!empty($usa_purchase_cash)){
	// 	$set['usa_krw_spot_selling_rate']=$usa_krw_spot_selling_rate;
	// }
		if(!empty($usa_gbp_purchase_cash)){
		$set['usa_gbp_purchase_cash']=$usa_gbp_purchase_cash;
		}
		if(!empty($usa_gbp_spot_selling_rate)){
		$set['usa_gbp_spot_selling_rate']=$usa_gbp_spot_selling_rate;
		}
		if(!empty($usa_aud_purchase_cash)){
		$set['usa_aud_purchase_cash']=$usa_aud_purchase_cash;
		}
		if(!empty($usa_aud_spot_selling_rate)){
		$set['usa_aud_spot_selling_rate']=$usa_aud_spot_selling_rate;
		}
		if(!empty($usa_cad_purchase_cash)){
        $set['usa_cad_purchase_cash']=$usa_cad_purchase_cash;
		}
		if(!empty($usa_cad_spot_selling_rate)){
		$set['usa_cad_spot_selling_rate']=$usa_cad_spot_selling_rate;
		}
		if(!empty($usa_chf_purchase_cash)){
		$set['usa_chf_purchase_cash']=$usa_chf_purchase_cash;
		}
		if(!empty($usa_chf_spot_selling_rate)){
		$set['usa_chf_spot_selling_rate']=$usa_chf_spot_selling_rate;
		}
		if(!empty($usa_nzd_purchase_cash)){
		$set['usa_nzd_purchase_cash']=$usa_nzd_purchase_cash;
		}
		if(!empty($usa_nzd_spot_selling_rate)){
		$set['usa_nzd_spot_selling_rate']=$usa_nzd_spot_selling_rate;
		}
		if(!empty($usa_sgd_purchase_cash)){
		$set['usa_sgd_purchase_cash']=$usa_sgd_purchase_cash;
		}
		if(!empty($usa_sgd_spot_selling_rate)){
		$set['usa_sgd_spot_selling_rate']=$usa_sgd_spot_selling_rate;
		}
		if(!empty($usa_hkc_purchase_cash)){
		$set['usa_hkc_purchase_cash']=$usa_hkc_purchase_cash;
		}
		if(!empty($usa_hkc_spot_selling_rate)){
		$set['usa_hkc_spot_selling_rate']=$usa_hkc_spot_selling_rate;
		}
		if(!empty($hkc_usa_purchase_cash)){
		$set['hkc_usa_purchase_cash']=$hkc_usa_purchase_cash;
		}
		if(!empty($hkc_usa_spot_selling_rate)){
		$set['hkc_usa_spot_selling_rate']=$hkc_usa_spot_selling_rate;
		}
		if(!empty($usa_btc_purchase_cash)){
		$set['usa_btc_purchase_cash']=$usa_btc_purchase_cash;
		}
		if(!empty($usa_btc_spot_selling_rate)){
		$set['usa_btc_spot_selling_rate']=$usa_btc_spot_selling_rate;
		}
		if(!empty($usa_eth_purchase_cash)){
		$set['usa_eth_purchase_cash']=$usa_eth_purchase_cash;
		}
		if(!empty($usa_eth_spot_selling_rate)){
		$set['usa_eth_spot_selling_rate']=$usa_eth_spot_selling_rate;
		}
		//----------modify end----------
		ConfigHandler::set('product',$set);
		$this->Messager("操作成功",'?mod=tttuangou&code=varshows');
	}
	...
	function Varedit(){
		...
		*/
		...
		if(is_numeric($default_lcbc))
		{
			$set['default_lcbc']=$default_lcbc;
			$set['default_lcbv']=bcdiv(bcdiv($default_lcbc,365,100),100,100);
		}


		//2015-12-10 modify----------
		if(is_numeric($default_dollar))
		{
			$set['default_dollar']=$default_dollar;
		}


		if(is_numeric($default_hkdollar))
		{
			$set['default_hkdollar']=$default_hkdollar;
		}
		if(is_numeric($default_euro))
		{
			$set['default_euro']=$default_euro;
		}
		if(is_numeric($default_jpy))
		{
			$set['default_jpy']=$default_jpy;
		}
		if(is_numeric($default_krw))
		{
			$set['default_krw']=$default_krw;
		}
		if(is_numeric($default_gbp))
		{
			$set['default_gbp']=$default_gbp;
		}
		if(is_numeric($default_aud))
		{
			$set['default_aud']=$default_aud;
		}
		if(is_numeric($default_cad))
		{
			$set['default_cad']=$default_cad;
		}
		if(is_numeric($default_chf))
		{
			$set['default_chf']=$default_chf;
		}
		if(is_numeric($default_nzd))
		{
			$set['default_nzd']=$default_nzd;
		}
		if(is_numeric($default_sgd))
		{
			$set['default_sgd']=$default_sgd;
		}
		if(is_numeric($default_hkc))
		{
			$set['default_hkc']=$default_hkc;
		}
		if(is_numeric($default_btc))
		{
			$set['default_btc']=$default_btc;
		}
		if(is_numeric($default_eth))
		{
			$set['default_eth']=$default_eth;
		}
		//----------modify end----------
		ConfigHandler::set('product',$set);
