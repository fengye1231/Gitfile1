...
class MasterObject
{
	...
	function MasterObject(&$config)
	{
		...
		foreach ($__navs as $i => $nav)
		{
			...
			{
				$curClass = 'current';
				$this->Config['__navs_index'] = $i+1;
			}
			elseif ($this->Module=='index' && $this->Code=='' && strpos($nav['url'], 'code')==false && trim($nav['url']) !=""  &&  substr($nav['url'], 0, 4) != 'http')
			{
				$curClass = 'current';
				$this->Config['__navs_index'] = $i+1;
			}elseif($this->Module=='catalog' && strpos($nav['url'], 'index'))
			{
								$curClass = 'current';
				$this->Config['__navs_index'] = $i+1;
			}
			...
			$index_navs[$i]=$__navs[$i];
		}
		$this->Config['__navs'] = $index_navs;
		
		if ($__navs[$this->Config['__navs_index']-1]['pid']!=0)
		{
			$this->Config['__navs'][$__navs[$this->Config['__navs_index']-1]['pid']-1]['class']="current";
			$this->Config['__navs_index']= $__navs[$this->Config['__navs_index']-1]['pid'];
		}
		
		foreach ($__navs as $i => $nav)
		{
			if($__navs[$i]['pid']!=$this->Config['__navs_index'])
			{
				continue;
			}
			$curClass = '';
			...
			$__navs[$i]['class'] = $curClass;
			$this->Config['__navs_two'][] = $__navs[$i];
		}
	
	foreach ($__navs as $i => $nav)
		{
			if($__navs[$i]['pid']!=$this->Config['__navs_san'])
			{
				continue;
			}
			$curClass = '';
			...
			$__navs[$i]['class'] = $curClass;
			$this->Config['__navs_san'][] = $__navs[$i];
		}
		...
		if($_SERVER['REQUEST_METHOD']=="POST" && $this->Module!='callback' && $this->Module!='misc' && $this->Module!='upload' && $this->Module!='address' && $this->Module!='me')
		{
			if($this->Post["FORMHASH"]!=FORMHASH || strpos($_SERVER["HTTP_REFERER"],$_SERVER["HTTP_HOST"])===false) { 
				//IAJAX || $this->Messager("请求无效",null);
			}
		}
